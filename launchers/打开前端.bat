...
REM    First match wins, but every additional match is called out so a stale
REM    second copy can never silently hijack the launch.
if not defined ROOT (
  for %%P in (
    "%USERPROFILE%\Desktop\biodata-agent"
    "%USERPROFILE%\OneDrive\Desktop\biodata-agent"
    "%USERPROFILE%\Downloads\biodata-agent"
    "%USERPROFILE%\Desktop\biodata-agent\biodata-agent"
    "%USERPROFILE%\OneDrive\Desktop\biodata-agent\biodata-agent"
  ) do (
    if exist "%%~P\scripts\run_web.py" (
      if not defined ROOT (set "ROOT=%%~P\" & set "GUESSED=1") else (echo [!] Another install also found at %%~P - ignored.)
    )
  )
)
if defined GUESSED echo [i] Using install at %ROOT%
